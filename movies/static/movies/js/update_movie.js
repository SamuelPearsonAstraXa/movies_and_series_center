const uplaod_movie_form = document.getElementById('update-movie-form');
...
description.setAttribute('placeHolder', 'Movie description');
...
producer.setAttribute('placeHolder', 'Movie producer');
...
document.querySelector(`label[for='id_banner_img']`).textContent = 'Movie Banner';
document.querySelector(`label[for='id_trailer']`).textContent = 'Movie trailer';

uplaod_movie_form.addEventListener('submit', e=>{
    e.preventDefault();
    loader.style.display = 'flex';
    
    fetch(uplaod_movie_form.action, {
        method: 'POST',
        body: new FormData(uplaod_movie_form),
        headers: {
    ...
    .then(data => {
        if (data.success){
            loader.style.display = 'none';
            document.getElementById('response_msg').innerHTML = `<p style='color:green;'>Your movie has been updated.</p>`
            setTimeout(() => {
    ...
    .catch(error => {
        ...
        loader.style.display = 'none';
        document.getElementById('response_msg').innerHTML = `<p style='color:red;'>There was an error while updating your movie.</p>`
    })
})