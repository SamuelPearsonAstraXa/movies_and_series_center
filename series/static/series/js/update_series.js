const uplaod_series_form = document.getElementById('update-series-form');
...
uplaod_series_form.addEventListener('submit', e=>{
    e.preventDefault();
    loader.style.display = 'flex';
    
    fetch(uplaod_series_form.action, {
        method: 'POST',
        body: new FormData(uplaod_series_form),
        headers: {
    ...
    .then(data => {
        if (data.success){
            loader.style.display = 'none';
            document.getElementById('response_msg').innerHTML = `<p style='color:green;'>Your series has been updated.</p>`
            setTimeout(() => {
    ...
    .catch(error => {
        ...
        loader.style.display = 'none';
        document.getElementById('response_msg').innerHTML = `<p style='color:red;'>There was an error while updating your series.</p>`
    })
})